...
for j=1:size(X,1)
    ...
    if ((~exist(fname_cluster2_sliding_ips,'file'))||(~exist(fname_cluster2_times,'file'))||(~exist(fname_cluster2_labels,'file'))||(~exist(fname_cluster2_waveforms,'file'))||(~exist(fname_cluster2_features,'file')))
        ...
        if (K1>0)
            ...
            inds00=find(tmp~=0);
            max_rad=max(abs(inds00-T_2));
            ttt=max(1,T_2-max_rad):min(T,T_2+max_rad-1);
            WF=WF0(1,ttt,:);
            ...
            for ii=1:length(critical_times)
                if ((critical_times(ii)-opts.clip_size)<1) use_it(ii)=0; end; %important to be conservative here because we extract the full clips below
                if ((critical_times(ii)+opts.clip_size)>N) use_it(ii)=0; end;
                k=ii-1;
            ...
            K=max(labels);
            WF0=zeros(size(X,1),opts.clip_size,K);
            clips_full_size=extract_clips(X,critical_times,opts.clip_size);
            for k=1:K
                inds=find(labels==k);
                %WF(:,:,k)=mean(clips(:,:,inds),3);
                WF0(:,:,k)=median(clips_full_size(:,:,inds),3);
            end;
            fprintf('Writing %s... ',fname_cluster2_waveforms);
            writemda(WF0,fname_cluster2_waveforms);
        else
            writemda(zeros(0,N),fname_cluster2_sliding_ips);
            writemda([],fname_cluster2_times);
            writemda(zeros(3,0),fname_cluster2_features);
            writemda([],fname_cluster2_labels);
            writemda([],fname_cluster2_waveforms);
        end;
    else
