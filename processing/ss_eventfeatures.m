function [z finfo] = ss_eventfeatures(X,o)
% snapshot of ahb's features on 5/22/2015
% SS_EVENTFEATURES - return feature vectors of chosen type from single events 3d array
%
% z = ss_eventfeatures(X,opts) where X is M x Nt x Ns 3d single-event data array returns
...
%   opts.fmethod =
%        'pca' PCA applied to signal vector across channels and times
...
%
% [z finfo] = ss_eventfeatures(X,opts) also returns finfo.subspace if 'pca', giving
...
if nargin<1, test_ss_eventfeatures; return; end
if nargin<2, o = []; end
...
%%%%%%%%%%
function test_ss_eventfeatures % some basic tests on feature extraction methods
% Barnett 12/19/14. std signs & real data added 12/22/14

if 0 % PCA features for random data
Ns = 1e4; X = randn(3,40,Ns); % Ns large
...
figure; plot(z(1:3,1:30)');
end

if 1  % try features of real dataset
  d = ss_load_test_data('b'); disp(d.name)
  d.A = ss_freqfilter(d.A,d.samplefreq,300,[]);
  o.quan = .01; o.samplefreq=d.samplefreq; [X t m info] = ss_detectevents(d.A,o); % old quantile-based detection
  fac = 1 % for 'e' makes little difference
  if fac>1, kerpars.Tf = 5; [X tu] = ss_upsample(X,fac,kerpars); end % upsample
  [Y ta] = ss_alignspikes(X,fac); [M T Ns] = size(Y);
  o.fmethod = 'pca';                      % fea meth: PCA
  z = ss_eventfeatures(Y,o);
